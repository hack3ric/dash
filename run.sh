...
# whether to use to epoch manager
epoch=(0 1 1 1 0)
...
# j specify the number of threads, 1 means one threads, 2 means two threads, 3 means four threads...
for k in 1; do
	for i in 1; do
		for j in 6; do
			echo "Begin: ${base[1]} ${workload[${i}]} ${thread_num[${j}]}"
			numaarg=""
			if [ "${thread_num[$j]}" -le 24 ]; then
				numaarg="--cpunodebind=0 --membind=0"
			elif [ "${thread_num[$j]}" -le 48 ]; then
